...
def bg_subtraction(inp_path,model_type,eval_path,out_path):
    ...
    if model_type==1:
        model = cv2.createBackgroundSubtractorMOG2(history=500,varThreshold=40,detectShadows=True)
    elif model_type==2:                                               ## chosing the model
        model = cv2.createBackgroundSubtractorKNN(dist2Threshold=800,detectShadows=True)
    elif model_type==3:
    ...
    for i in range(len(image_list)):
        ...
        # name=f"{i}.png"
        # cv2.imwrite(os.path.join(out_path,name),frame)
